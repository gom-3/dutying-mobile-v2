...
async function copyResourceFiles(widgetSourceDir: string, platformRoot: string) {
...
async function prepareSourceCodes(
  ...
  if (!fs.existsSync(source)) {
    ...
    await fs.promises.cp(templateFolder, source, { recursive: true });
    await fs.promises.rename(path.join(source, 'SampleWidget.kt'), widgetSourceFilePath);
