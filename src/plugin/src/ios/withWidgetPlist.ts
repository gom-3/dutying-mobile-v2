import { type ConfigPlugin, type InfoPlist, withDangerousMod } from '@expo/config-plugins';
...
export const withWidgetPlist: ConfigPlugin<Props> = (config, { targetName }) => {
  return withDangerousMod(config, [
    ...
    async (config) => {
      ...
      await fs.promises.writeFile(extensionPlistPath, plist.build(extensionPlist));
