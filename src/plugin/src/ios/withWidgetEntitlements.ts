import { type ConfigPlugin, type InfoPlist, withXcodeProject } from '@expo/config-plugins';
...
export const withWidgetEntitlements: ConfigPlugin<Props> = (
  ...
  return withXcodeProject(config, async (config) => {
    ...
    await fs.promises.writeFile(entitlementsPath, plist.build(extensionEntitlements));
