...
import { type SearchFriendCodeResponseDTO, searchFriendCode } from '@/api/friend';
...
const RequestFriendPage = () => {
  ...
  const { mutate: searchFriendMutate } = useMutation({
    mutationFn: (code: string) => searchFriendCode(code),
    onSuccess: (data) => {
      setFriend(data);
      setIsModalOpen(true);
  ...
  const pressEnterButton = () => {
    if (enteredInput.length !== 6) {
      ...
      searchFriendMutate(enteredInput);
    }
  };
