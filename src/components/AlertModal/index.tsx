...
import { View, Text, TouchableOpacity, Pressable, Share, Image, StyleSheet } from 'react-native';
import Modal from 'react-native-modal';
import Toast from 'react-native-toast-message';
import { sendRequestFriend } from '@/api/friend';
import { type SearchMoimFromCodeResponseDTO, joinMoim } from '@/api/moim';
...
export const AlertModalRequestFriend = ({
  ...
  const { mutate: sendRequestFriendMutate } = useMutation({
    mutationFn: ({ accountId }: { accountId: number }) => sendRequestFriend(accountId),
    onSuccess: () => {
      navigate.goBack();
      Toast.show({
        type: 'success',
  ...
  const pressEnterButton = () => {
    close();
    sendRequestFriendMutate({ accountId });
  };
...
export const AlertModalEnter = ({ isOpen, close, moim, accountId }: EnterProps) => {
  ...
  const { mutate: enterMoimMutate } = useMutation({
    mutationFn: ({ moimId, accountId }: { moimId: number; accountId: number }) =>
      joinMoim(moimId, accountId),
    onSuccess: () => {
  ...
  const pressEnterButton = () => {
    close();
    enterMoimMutate({ moimId: moim.moimId, accountId });
  };
